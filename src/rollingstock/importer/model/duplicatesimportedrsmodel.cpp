...
public:
    ...
    void run() override
    {
        ...
        query q(
          mDb,
          "SELECT COUNT(1) FROM ("
          "SELECT (CASE WHEN imp.new_number NOT NULL THEN imp.new_number ELSE imp.number END) AS "
          "num,"
        ...
        q.prepare(
          "SELECT imp.id, mod1.id, mod1.match_existing_id, mod1.type,"
        ...
        for (auto r : q)
        {
            ...
            if (r.column_type(6) != SQLITE_NULL)
            {
                ...
                QByteArray tmp = QByteArray::fromRawData(
                  reinterpret_cast<char const *>(sqlite3_column_text(st, 6)),
            ...
            if (r.column_type(12) != SQLITE_NULL)
            {
                // 'name (match_existing name)'
                QByteArray tmp = QByteArray::fromRawData(
                  reinterpret_cast<char const *>(sqlite3_column_text(st, 12)),
...
QVariant DuplicatesImportedRSModel::headerData(int section, Qt::Orientation orientation,
                                               int role) const
{
...
bool DuplicatesImportedRSModel::setData(const QModelIndex &idx, const QVariant &value, int role)
{
    ...
    case Qt::EditRole:
    {
        if (idx.column() == NewNumber)
        {
            ...
            QString errText;
            if (!iface->checkNewNumberIsValid(item.importedId, item.importedModelId,
                                              item.matchExistingModelId, item.type, item.number,
                                              newNumber, &errText))
            {
                emit error(errText);
                return false;
