...
void ImportTask::run()
{
    ...
    sqlite3pp::query q(mDb, "SELECT imp.id,imp.name,imp.new_name FROM imported_rs_owners imp WHERE imp.import=1 AND imp.match_existing_id IS NULL");
    sqlite3pp::command q_create(mDb, "INSERT INTO rs_owners(id, name) VALUES(NULL, ?)");
    sqlite3pp::command q_update(mDb, "UPDATE imported_rs_owners SET match_existing_id=? WHERE id=?");
    ...
    q.prepare("SELECT imp.id,imp.name,imp.suffix,imp.new_name,imp.max_speed,imp.axes,imp.type,imp.sub_type"
              " FROM imported_rs_models imp WHERE imp.import=1 AND imp.match_existing_id IS NULL");
    st = q.stmt();

    q_create.prepare("INSERT INTO rs_models(id,name,suffix,max_speed,axes,type,sub_type) VALUES(NULL,?,?,?,?,?,?)"); //Create invalid engine
    ...
    q_create.prepare("INSERT INTO rs_list(id, model_id, number, owner_id)"
                     " SELECT NULL, m.match_existing_id, (CASE WHEN imp.new_number IS NULL THEN imp.number ELSE imp.new_number END), o.match_existing_id"
