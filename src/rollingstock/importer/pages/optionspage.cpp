    ...
    optionsWidget(nullptr)
{
    ...
    defaultSpeedSpin->setToolTip(
      tr("Default speed is applied when a rollingstock model is not matched to an existing one"
    ...
    defaultTypeCombo->setToolTip(
      tr("Default type is applied when a rollingstock model is not matched to an existing one"
         " and has to be created from scratch"));
    ...
    connect(backendCombo, static_cast<void (QComboBox::*)(int)>(&QComboBox::activated), this,
            &OptionsPage::setSource);
...
void OptionsPage::updateGeneralCheckBox()
{
    ...
    else
    {
        ...
        {
            QMessageBox::warning(
              this, RsImportStrings::tr("No rolloingstock imported"),
              RsImportStrings::tr(
                "No rollingstock piece will be imported.\n"
        ...
        importRS->setToolTip(RsImportStrings::tr(
          "In order to import rollingstock pieces you must also import models and owners."));
    }
    importRS->blockSignals(false);
