...
RSModelsMatchModel::RSModelsMatchModel(database &db, QObject *parent) :
    ...
    q_getMatches(mDb)
{
    q_getMatches.prepare("SELECT id,name,suffix FROM rs_models WHERE name LIKE ?1 OR suffix LIKE ?1 LIMIT " QT_STRINGIFY(MaxMatchItems + 1));
}
...
void RSModelsMatchModel::refreshData()
{
    ...
    for(; i < MaxMatchItems && it != end; i++)
    {
        ...
        const char *name = reinterpret_cast<const char *>(sqlite3_column_text(q_getMatches.stmt(), 1));

        int suffixLength = sqlite3_column_bytes(q_getMatches.stmt(), 2);
        const char *suffix = reinterpret_cast<const char *>(sqlite3_column_text(q_getMatches.stmt(), 2));
...
{
    return items[row].nameWithSuffix.left(items[row].nameLength); //Remove the suffix, keep only the name
}
