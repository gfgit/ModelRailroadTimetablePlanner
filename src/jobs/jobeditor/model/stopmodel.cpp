    ...
    autoUncoupleAtLast(true)
{
    ...
    connect(Session, &MeetingSession::segmentNameChanged, this,
            &StopModel::onStationSegmentNameChanged);
...
bool StopModel::loadJobStops(db_id jobId)
{
    ...
    q_selectStops.prepare(
      "SELECT stops.id, stops.station_id, stops.arrival, stops.departure, stops.type,"
    ...
    for (auto stop : q_selectStops)
    {
        ...
        s.fromGate.gateTrackNum     = stop.get<int>(7);
        ...
        s.toGate.gateTrackNum       = stop.get<int>(12);
        ...
        if (s.trackId != otherTrackId && s.toGate.gateConnId)
        {
            ...
            qWarning() << "Stop:" << s.stopId << "Different track:" << s.fromGate.gateConnId
                       << s.toGate.gateConnId;
        }
        if (prevSegment.segmentId != 0)
        {
            if (prevOutGateId != s.fromGate.gateId)
            {
                ...
                qWarning() << "Stop:" << s.stopId
                           << "Different prev segment:" << prevSegment.segConnId
                           << s.fromGate.gateConnId;
            }
            ...
            if (s.fromGate.gateTrackNum != prevSegment.outTrackNum)
            {
                ...
                qWarning() << "Stop:" << s.stopId
                           << "Different in gate track:" << s.fromGate.gateConnId
                           << s.toGate.gateConnId;
            }
        }
        if (s.nextSegment.segmentId != 0)
        {
            if (segInGateId != s.toGate.gateId)
            {
                ...
                qWarning() << "Stop:" << s.stopId
                           << "Different next segment:" << s.nextSegment.segConnId
                           << s.toGate.gateConnId;
            }
            ...
            if (s.toGate.gateTrackNum != s.nextSegment.inTrackNum)
            {
                ...
                qWarning() << "Stop:" << s.stopId
                           << "Different out gate track:" << s.toGate.gateConnId
                           << s.nextSegment.segConnId;
            }
        }
        ...
        if (i == 0)
        {
            ...
            if (stopType != 0)
            {
                ...
                qWarning() << "Error: First stop cannot be transit! Job:" << mJobId
                           << "StopId:" << s.stopId;
            }
        }
        else if (stopType)
        {
            ...
            if (i == count - 1)
            {
                ...
                qWarning() << "Error: Last stop cannot be transit! Job:" << mJobId
                           << "StopId:" << s.stopId;
            }
        }
...
bool StopModel::revertChanges()
{
    ...
    if (editState == StopsEditing)
    {
        ...
        if (ret != SQLITE_OK)
        {
            qDebug() << "Error while clearing current stops:" << ret << mDb.error_msg()
        ...
        if (ret != SQLITE_OK)
        {
            qDebug() << "Error while restoring old stops:" << ret << mDb.error_msg()
                     << mDb.extended_error_code();
            return false;
        }
        ...
        cmd.prepare("INSERT INTO coupling(id, stop_id, rs_id, operation)"
                    " SELECT old_coupling.id, old_coupling.stop_id, old_coupling.rs_id, "
                    "old_coupling.operation"
                    " FROM old_coupling"
        ...
        if (ret != SQLITE_OK)
        {
            qDebug() << "Error while restoring old couplings:" << ret << mDb.error_msg()
                     << mDb.extended_error_code();
            return false;
        }
...
void StopModel::addStop()
{
    ...
    if (prevIdx >= 0) // Has stops before
    {
        ...
        if (autoMoveUncoupleToNewLast)
        {
            if (prevIdx >= 1)
            {
                ...
                command q_moveUncoupled(
                  mDb, "UPDATE OR IGNORE coupling SET stop_id=? WHERE stop_id=? AND operation=?");
                ...
                if (ret != SQLITE_OK)
                {
                    qDebug() << "Error shifting uncoupling from stop:" << s.stopId
                             << "to:" << last.stopId << "Job:" << mJobId << "err:" << ret
                             << mDb.error_msg();
                }
            }
        ...
        emit dataChanged(index(prevIdx, 0), index(prevIdx, 0));
    }
    else
    {
    ...
    insertAddHere(
      idx + 1,
      1); // Insert only at end because may invalidate StopItem& references due to mem realloc
}
...
void StopModel::removeStop(const QModelIndex &idx)
{
    ...
    else if (s.type == StopType::Last)
    {
        ...
        {
            // We are new last stop and previous is not First (>= 1)
            // Move couplings from former last stop (now removed) to new last stop (former last but
            // one)
            ...
            {
                qDebug() << "Error shifting uncoupling from stop:" << s.stopId
                         << "to:" << prev.stopId << "Job:" << mJobId << "err:" << ret
                         << mDb.error_msg();
            }
        ...
        // Clear previous 'out' gate and next segment, set type to Normal, reset Departure so it's
        // equal to Arrival
        command cmd(
          mDb, "UPDATE stops SET "
               "out_gate_conn=NULL,next_segment_conn_id=NULL,type=?,departure=arrival WHERE id=?");
        cmd.bind(1, int(StopType::Normal));
...
void StopModel::uncoupleStillCoupledAtStop(const StopItem &s)
{
    ...
    command q_uncoupleRS(
      mDb, "INSERT OR REPLACE INTO coupling(id,stop_id,rs_id,operation) VALUES(NULL,?,?,0)");
                            ...
                            bool avoidTimeRecalc)
{
    ...
    if (s.type != StopType::First && row > 0)
    {
        ...
        if (prevStop.nextSegment.segmentId != prevSeg.segmentId)
        {
            ...
            {
                qWarning() << "StopModel: cannot update previous stop segment:" << mDb.error_msg()
                           << prevStop.stopId;
                return;
            }
            ...
            newStop.fromGate.gateTrackNum = prevStop.nextSegment.outTrackNum;
        }
    }
    ...
    if (s.toGate.gateConnId != newStop.toGate.gateConnId
        || oldSegConnId != newStop.nextSegment.segConnId)
    {
...
bool StopModel::setStopTypeRange(int firstRow, int lastRow, StopType type)
{
    ...
    for (int r = firstRow; r <= lastRow; r++)
    {
        ...
        {
            qWarning() << "Error: tried change type of First/Last stop:" << r << s.stopId
                       << "Job:" << mJobId;
        ...
        if (destType == StopType::Transit)
        {
            ...
            if (res == SQLITE_ROW)
            {
                qWarning() << "Error: trying to set Transit on stop:" << s.stopId
                           << "Job:" << mJobId << "while having coupling operation for this stop";
                canBeTransit = false;
            }
            if (res != SQLITE_OK && res != SQLITE_DONE)
            {
                qWarning() << "Error while setting stopType for stop:" << s.stopId
                           << "Job:" << mJobId << "DB Err:" << res << mDb.error_msg()
                           << mDb.extended_error_code();
                return false;
            }
        }
...
bool StopModel::trySelectTrackForStop(StopItem &item)
{
    ...
    if (item.fromGate.gateId)
    {
        ...
        // Try to keep previous selected station track otherwise choose lowest position track
        // possible
    ...
    item.fromGate.gateTrackNum     = gate.get<int>(2);
...
bool StopModel::trySetTrackConnections(StopItem &item, db_id trackId, QString *outErr)
{
    ...
    if (item.type == StopType::First)
    {
        ...
        q.prepare("SELECT id,gate_id,gate_track,track_side FROM station_gate_connections WHERE "
                  "track_id=? LIMIT 1");
        q.bind(1, trackId);
        if (q.step() != SQLITE_ROW)
        {
    ...
    if (item.toGate.gateConnId)
    {
        ...
        q.bind(2, item.toGate.gateTrackNum);
        q.bind(3, trackId);
        ...
        else
        {
            ...
            if (outErr)
                *outErr =
                  tr("Track is not connected to selected out gate track.\n"
...
bool StopModel::trySelectNextSegment(StopItem &item, db_id segmentId, int suggestedOutGateTrk,
                                     db_id nextStationId, db_id &seg_out_gateId,
                                     db_id &out_suggestedTrackId)
{
    ...
    if (seg_in_gateId != item.toGate.gateId
        || item.toGate.gateTrackNum != item.nextSegment.inTrackNum)
    {
        ...
        if (q.step() != SQLITE_ROW)
        {
            ...
            // User must change previous track, make a suggestion (lowest track connected to in and
            // out gates)
            ...
            if (item.type != StopType::First)
            {
                // Check also in gate
                sql.replace("%3",
                            "JOIN station_gate_connections c1 ON"
        ...
        item.toGate.gateTrackNum     = conn.get<int>(1);
        ...
        item.nextSegment.inTrackNum  = item.toGate.gateTrackNum;
...
void StopModel::deleteStop(db_id stopId)
{
    ...
    if (ret != SQLITE_OK)
    {
        qWarning() << "DB err:" << ret << mDb.error_code() << mDb.error_msg()
                   << mDb.extended_error_code();
    }
    q_removeStop.reset();
}
...
void StopModel::rebaseTimesToSpeed(int firstIdx, QTime firstArr, QTime firstDep)
{
    ...
    {
        firstDep =
          firstArr.addSecs(60); // At least 1 minute stop, it cannot be a transit if we couple RS
    }
    ...
    for (int idx = firstIdx + 1; idx < stops.size() - 1; idx++)
    {
        ...
        // If line is slower or we couldn't get trains speed (likely no RS coupled) use line max
        // speed instead
...
bool StopModel::updateCurrentInGate(StopItem &curStop, const StopItem::Segment &prevSeg)
{
    ...
    if (!curStop.fromGate.gateConnId)
    {
        ...
        curStop.fromGate.gateTrackNum = prevSeg.reversed ? segInTrack : segOutTrack;
        ...
        if (curStop.fromGate.gateId != oldGateId || curStop.fromGate.gateTrackNum != oldTrackNum)
        {
...
bool StopModel::updateStopTime(StopItem &item, int row, bool propagate, const QTime &oldArr,
                               const QTime &oldDep)
{
...
{
    // HACK: when applying msecOffset to stops query might not work because there might be a (next)
    // stop with the same arrival
    //       that is being set and thus SQL hits UNIQUE constraint and rejects the modification
    // SOLUTION: shift all subsequent stops by 24 hours so there will be no conflicts and then reset
    // the time once at a time
    //           so in the end they all will have correct time (no need to shift backwards)

    command q_shiftArrDep(
      mDb,
      "UPDATE stops SET arrival=arrival+?1,departure=departure+?1 WHERE job_id=?2 AND arrival>?3");
...
bool StopModel::startInfoEditing() // Faster than 'startStopEditing()' use this if change doesen.t
                                   // affect stops
{
    if (editState != NotEditing)
        return true;
...
bool StopModel::startStopsEditing()
{
    ...
    if (ret != SQLITE_OK)
    {
        qWarning() << "Error while saving old stops:" << ret << mDb.error_msg()
                   << mDb.extended_error_code();
        return false;
    }
    ...
    if (ret != SQLITE_OK)
    {
        qDebug() << "Error while saving old couplings:" << ret << mDb.error_msg()
                 << mDb.extended_error_code();
        return false;
    }
...
bool StopModel::endStopsEditing()
{
    ...
    if (editState == StopsEditing)
    {
        ...
        if (ret != SQLITE_OK)
        {
            qDebug() << "Error while clearing old stops:" << ret << mDb.error_msg()
                     << mDb.extended_error_code();
            return false;
        }
    }
