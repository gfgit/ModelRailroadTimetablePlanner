...
IPagedItemModelImpl<SuperType, ModelItemType>::IPagedItemModelImpl(const int itemsPerPage,
...
void IPagedItemModelImpl<SuperType, ModelItemType>::handleResult(const Cache &items, int firstRow)
{
    ...
    int lastRow =
      firstRow + itemsCopy.count(); // Last row + 1 extra to re-trigger possible next batch
    ...
    if (firstRow > 0)
        firstRow--; // Try notify also the row before because there might be another batch waiting
                    // so re-trigger it
