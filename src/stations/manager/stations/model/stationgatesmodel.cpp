...
// Error messages
static constexpr char errorNameNotLetterText[] =
  QT_TRANSLATE_NOOP("StationGatesModel", "Gate name must be a letter.");

static constexpr char errorNameAlreadyUsedText[] =
  QT_TRANSLATE_NOOP("StationGatesModel",
                    "This gate letter <b>%1</b> is already used by another gate of this station.");

static constexpr char errorGateInUseText[] =
  QT_TRANSLATE_NOOP("StationGatesModel", "This gate is still referenced.\n"
                                         "Please remove all references before deleting it.");

static constexpr char errorGateEntranceOrExitText[] = QT_TRANSLATE_NOOP(
  "StationGatesModel", "The gate must be at least Entrance or Exit.\n"
...
bool StationGatesModel::getStationInfo(QString &name, QString &shortName, utils::StationType &type,
                                       qint64 &phoneNumber, bool &hasImage) const
{
    query q(mDb,
            "SELECT name,short_name,type,phone_number,svg_data IS NULL FROM stations WHERE id=?");
    q.bind(1, m_stationId);
    if (q.step() != SQLITE_ROW)
        return false;
    auto r    = q.getRows();
...
bool StationGatesModel::setStationInfo(const QString &name, const QString &shortName,
...
void StationGatesModel::internalFetch(int first, int sortCol, int valRow, const QVariant &val)
{
    ...
    QByteArray sql = "SELECT g.id,g.out_track_count,g.type,g.def_in_platf_id,g.name,g.side,t.name "
                     "FROM station_gates g"
...
bool StationGatesModel::setDefaultPlatf(StationGatesModel::GateItem &item, db_id trackId,
                                        const QString &trackName)
{
    if (item.defaultInPlatfId == trackId)
        return false;

    // FIXME: restrict popup suggestions to tracks connected to this gate and not all station's
    // tracks
