...
bool loadStationTrackConnections(sqlite3pp::database &db, db_id stationId,
...
bool StationSVGHelper::getPrevNextStop(sqlite3pp::database &db, db_id stationId, bool next,
                                       QTime &time)
{
...
bool StationSVGHelper::applyStationJobsToPlan(const StationSVGJobStops *station,
...
bool StationSVGHelper::writeStationXmlFromDB(sqlite3pp::database &db, db_id stationId,
                                             QIODevice *dev)
{
    ssplib::StationPlan plan;
...
bool StationSVGHelper::importTrackConnFromSVG(sqlite3pp::database &db, db_id stationId,
                                              ssplib::StationPlan *plan)
{
    query q_getTrack(db, "SELECT id FROM station_tracks WHERE station_id=? AND pos=?");
    query q_getGate(db,
                    "SELECT id, out_track_count FROM station_gates WHERE station_id=? AND name=?");
    ...
    for (const auto &conn : std::as_const(plan->trackConnections))
    {
        ...
        if (ret != SQLITE_ROW)
        {
            qWarning() << "Error importing connection from SVG, station" << stationId
                       << "has no track at pos" << conn.info.stationTrackPos;
            continue;
        }
        ...
        {
            qWarning() << "Error importing connection from SVG, station" << stationId
                       << "has no gate" << conn.info.gateLetter;
            continue;
        }
        ...
        {
            qWarning() << "Error importing connection from SVG, station" << stationId << "\n"
                       << "gate" << conn.info.gateLetter << gateId << "has only" << gateTrkCnt
                       << "tracks\n"
...
bool StationSVGHelper::importTrackConnFromSVGDev(sqlite3pp::database &db, db_id stationId,
                                                 QIODevice *dev)
{
    ssplib::StationPlan plan;
