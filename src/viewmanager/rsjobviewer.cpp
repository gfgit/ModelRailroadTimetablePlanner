...
void RSJobViewer::updateRsInfo()
{
    query q_getRSInfo(
      Session->m_Db,
      "SELECT rs_list.number,rs_models.name,rs_models.suffix,rs_models.type,rs_owners.name"
      " FROM rs_list"
    ...
    const QString name =
      rs_utils::formatNameRef(modelName, modelNameLen, number, modelSuffix, modelSuffixLen, type);
    setWindowTitle(name);

    const QString info = tr("Type:  <b>%1</b><br>"
                            "Owner: <b>%2</b>")
                           .arg(RsTypeNames::name(type));
