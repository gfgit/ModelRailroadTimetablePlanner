...
QVariant SessionStartEndModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (orientation == Qt::Horizontal && role == Qt::DisplayRole)
    {
        ...
        case StationOrOwnerCol:
            return m_order == SessionRSOrder::ByStation ? tr("Owner")
                                                        : tr("Station"); // Opposite of order
        default:
            break;
        }
    }
...
QModelIndex SessionStartEndModel::index(int row, int column, const QModelIndex &p) const
{
    if (p.isValid())
    {
        ...
        if (row < 0 || (row + parents[parentRow].firstIdx) >= rsData.size() || column < 0
            || column >= NCols)
            return QModelIndex();
...
void SessionStartEndModel::setMode(SessionRSMode m, SessionRSOrder o, bool forceReload)
{
    ...
    const auto sql = QStringLiteral(
      "SELECT %1(stops.arrival), stops.station_id, rs_list.owner_id,"
    ...
    QByteArray query =
      sql
        .arg(m_mode == SessionRSMode::StartOfSession ? "MIN" : "MAX",
